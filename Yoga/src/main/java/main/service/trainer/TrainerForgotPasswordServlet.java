...
/**
 * Servlet implementation class TrainerForgotPasswordServlet
 */
public class TrainerForgotPasswordServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String traineremail = (String) request.getParameter("username");
		...
		try (Connection con = DBConnection.createConnection()) {
			...
			String query2 = "update trainerregister set password=? where traineremail=?";
			try (PreparedStatement statement2 = con.prepareStatement(query2)) {
				statement2.setString(1, password);
				statement2.setString(2, traineremail);

				j = statement2.executeUpdate();
				if (j != 0) {
					out.write("NewPassword Sent");

					System.out.println("Trainer NewPassword Sent");
